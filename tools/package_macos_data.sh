...
# 注入警察故事 2 繁中資料(dist-cht/ 的 translation.tsv + qfg1_big5.fnt + qfg1_big5_hi.fnt
# + pq2_title.ovl)+ README,重新打包成可交付檔。在 CI runner 內跑(bash 內建即可,不需 docker/python)。
#
# PQ2 只有單一 SCI0 EGA 版本(不像 QFG1/LSL1 要分 vga/ega),故不帶 edition 參數。
#
...
[ -f "$DATA_DIR/translation.tsv" ] || { echo "!! 找不到中文資料（$ROOT/dist-cht 或 $ROOT/game）" >&2; exit 1; }
cp "$DATA_DIR/translation.tsv" "$CHT_DIR/"
cp "$DATA_DIR"/*.fnt "$CHT_DIR/"
...
- cht-data/：中文資料（translation.tsv 對白/訊息、qfg1_big5.fnt 低解析字型、qfg1_big5_hi.fnt hi-res 字型、pq2_title.ovl 中文標題疊圖）
- 本說明檔

本包【不含】原遊戲資源，也不含 Roland MT-32 ROM（版權因素不隨包分發）。
請自備合法取得的警察故事 2（SCI/DOS 版）遊戲檔。
...
repo：https://github.com/wicanr2/police_quest2_cht.git
EOF

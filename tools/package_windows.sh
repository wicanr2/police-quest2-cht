...
# PQ2 只有一個版本(SCI0 EGA),不像 LSL1/QFG1 要分 ega/vga——用 --auto-detect 免指定 target。
...
OUT="$DIST/PQ2-CHT-win64.zip"

[ -f "$EXE" ] || { echo "!! 找不到 $EXE（先跑 mingw build：docker run ... pq2-mingw make）"; exit 1; }
...
docker run --rm --name pq2-winpkg-strip -v "$STAGE:/s" "$MINGW_IMG" x86_64-w64-mingw32-strip /s/scummvm.exe

echo ">> 收集 mingw runtime DLL（objdump 確認只需 SDL2.dll + libwinpthread-1.dll，其餘皆系統內建）"
docker run --rm --name pq2-winpkg-sdl2dll "$MINGW_IMG" cat /usr/x86_64-w64-mingw32/bin/SDL2.dll > "$STAGE/SDL2.dll"
docker run --rm --name pq2-winpkg-pthreaddll "$MINGW_IMG" cat /usr/x86_64-w64-mingw32/lib/libwinpthread-1.dll > "$STAGE/libwinpthread-1.dll"
...
警察故事 2：復仇記（Police Quest II: The Vengeance）繁體中文化 — Windows x86_64 完整包

雙擊「玩-警察故事2-繁中.bat」即可開始遊戲。
...
repo（patch-only，不含遊戲資源/ROM）：https://github.com/wicanr2/police_quest2_cht
TXT

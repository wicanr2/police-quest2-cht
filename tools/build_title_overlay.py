#!/usr/bin/env python3
"""烘 PQ2 中文標題副標疊圖 pq2_title.ovl（引擎 drawPicture 時 blit 到 640x400 display）。
渲染「復仇記」金色點陣（配合 Police Quest II logo 的金色調）→ 量化到 EGA 16 色調色盤 →
輸出 .ovl：u16LE width,height,x,y（display 640x400 座標）+ width*height bytes EGA index(0-15)，
0xFF=透明。純輸出，字型用 Pillow。

用法：build_title_overlay.py <out.ovl> [--text 復仇記] [--font PATH] [--face IDX]
"""
...
def main():
    ...
    ap.add_argument("out")
    ap.add_argument("--text",default="復仇記")
    ...
    ap.add_argument("--y",type=int,default=320)   # 標題圖下方

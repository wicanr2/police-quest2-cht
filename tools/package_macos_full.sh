...
#
# 前提：先從 CI artifact 下載 engine-only tar.gz（gh run download ... --name pq2-cht-macos）。
...
GAME_SRC="${2:-/home/anr2/scummvm/police_quest2/workplace/game}"
ROM_SRC="${3:-/home/anr2/cht/mt32}"
OUT="/home/anr2/scummvm/police_quest2/dist-all/macos"
...
# 3) 啟動包裝：binary 改名 + wrapper 帶 PQ2 中文 + MT-32 參數
...
# 6) 打包
mkdir -p "$OUT"
( cd "$WORK" && tar czf "$OUT/PQ2-CHT-macos-universal-full.tar.gz" "ScummVM.app" "修復-macOS.command" )
echo "完成 → $OUT/PQ2-CHT-macos-universal-full.tar.gz ($(du -h "$OUT/PQ2-CHT-macos-universal-full.tar.gz" | cut -f1))"
rm -rf "$WORK"
